      ...
      {tab === 'overview' && (
        <div className="space-y-5 animate-fade-in">
            ...
            <GlassCard className="p-5">
              ...
              <div className="grid grid-cols-4 gap-3">
                <div className="glass-subtle rounded-xl p-4 text-center">
                  <div className={`text-sm font-bold ${health.database?.status === 'online' ? 'text-green-600' : 'text-red-500'}`}>{health.database?.status === 'online' ? 'Online' : 'Error'}</div>
                  <div className="text-[10px] text-gray-400 mt-0.5">Database</div>
                </div>
                <div className="glass-subtle rounded-xl p-4 text-center">
                  <div className="text-sm font-bold text-blue-600">{health.system?.cpu_percent || 0}%</div>
                  <div className="text-[10px] text-gray-400 mt-0.5">CPU</div>
                </div>
                <div className="glass-subtle rounded-xl p-4 text-center">
                  <div className="text-sm font-bold text-amber-600">{health.system?.memory_percent || 0}%</div>
                  <div className="text-[10px] text-gray-400 mt-0.5">Memory</div>
                </div>
                <div className="glass-subtle rounded-xl p-4 text-center">
                  <div className="text-sm font-bold text-purple-600">{health.uptime_hours || 0}h</div>
          ...
          <GlassCard className="p-5">
            ...
            <p className="text-gray-400 text-xs mb-2">One per line: name, email, password (password optional)</p>
            ...
            {bulkResult && <div className="mt-2 px-3 py-2 bg-green-50 rounded-lg border border-green-500/15 text-green-600 text-xs">Created: {bulkResult.created} · Skipped: {bulkResult.skipped}</div>}
          </GlassCard>
      ...
      {tab === 'users' && (
        <div className="space-y-4 animate-fade-in">
          <div className="flex justify-between items-center">
            <h3 className="text-base font-semibold text-slate-800">User Management ({users.length})</h3>
            ...
            <div className="space-y-1.5">
              {users.map(u => (
                <div key={u.id} className="flex items-center gap-3 p-3 glass-subtle rounded-xl">
                  <div className="w-9 h-9 rounded-lg bg-gradient-to-br from-indigo-100 to-purple-100 border border-indigo-200 flex items-center justify-center text-slate-800 text-xs font-bold flex-shrink-0">{u.name?.charAt(0)||'?'}</div>
                  ...
                  <select value={u.role} onChange={e => toggleRole(u.id, e.target.value)} className="px-2 py-1 rounded-lg bg-gray-50 border border-gray-200 text-slate-800 text-xs focus:outline-none">
                    <option value="student">Student</option><option value="teacher">Teacher</option><option value="admin">Admin</option>
                  </select>
                  <div className="text-center flex-shrink-0"><div className="text-xs font-bold text-slate-800">{u.sessions||0}</div><div className="text-[9px] text-gray-400">Sessions</div></div>
                  <button onClick={() => toggleStatus(u.id)} className={`px-2.5 py-1 rounded-lg text-[10px] font-semibold ${u.is_active ? 'bg-green-50 text-green-600 border border-green-200' : 'bg-red-50 text-red-500 border border-red-200'}`}>{u.is_active ? 'Active' : 'Disabled'}</button>
                </div>
              ))}
            </div>
          </GlassCard>
        ...
        <div className="space-y-5 animate-fade-in">
          <div className="flex gap-2 flex-wrap">
            {STATS_SUB_TABS.map(t => (
              <button key={t.id} onClick={() => setStatsSubTab(t.id)}
                className={`px-3 py-1.5 rounded-xl text-xs font-medium transition-all ${statsSubTab === t.id ? 'bg-indigo-50 text-indigo-600 border border-indigo-200' : 'bg-gray-50 text-gray-500 border border-gray-200 hover:bg-gray-50'}`}>
                {t.label}
              </button>
            ))}
          </div>
                ...
                <div className="space-y-2">
                  {(institution.courses || []).map((c, i) => (
                    <div key={c.id} className="flex items-center gap-3 p-3 glass-subtle rounded-xl">
                      ...
                      <div className="flex-1"><div className="text-sm text-slate-800 font-medium">{c.name}</div><div className="text-[10px] text-gray-400">by {c.teacher} · {c.enrolled} students · {c.total_sessions} sessions</div></div>
                      <div className="w-28"><div className="flex justify-end mb-0.5"><span className="text-xs font-bold" style={{color:engColor(c.avg_engagement)}}>{c.avg_engagement}%</span></div><div className="h-1.5 bg-gray-50 rounded-full overflow-hidden"><div className="h-full rounded-full" style={{width:`${c.avg_engagement}%`, background:engColor(c.avg_engagement)}}/></div></div>
                    </div>
                  ))}
                </div>
              </GlassCard>
          ...
          {statsSubTab === 'model' && modelPerf && (
            <div className="space-y-4">
              <div className="grid grid-cols-4 gap-3">
                {[
                  { label: 'Total Detections', value: modelPerf.total_detections, color: 'text-slate-800' },
                  { label: 'Avg Confidence', value: `${modelPerf.avg_confidence}%`, color: 'text-green-600' },
                  { label: 'High Conf', value: `${modelPerf.high_confidence_pct}%`, color: 'text-blue-600' },
                  { label: 'Low Conf', value: `${modelPerf.low_confidence_pct}%`, color: 'text-red-500' },
                ].map(s => (
                  <GlassCard key={s.label} className="p-4 text-center"><div className={`text-xl font-bold ${s.color}`}>{s.value}</div><div className="text-[10px] text-gray-400 mt-0.5">{s.label}</div></GlassCard>
                ))}
              </div>
              <GlassCard className="p-5">
                ...
                <div className="space-y-1.5">{Object.entries(modelPerf.emotion_distribution||{}).sort((a,b)=>b[1]-a[1]).map(([emo,cnt]) => { const pct = modelPerf.total_detections>0?(cnt/modelPerf.total_detections*100):0; return (
                  <div key={emo}><div className="flex justify-between text-xs mb-0.5"><span className="text-gray-400 capitalize">{emo}</span><span className="text-slate-800 font-bold">{cnt} ({pct.toFixed(1)}%)</span></div><div className="h-1.5 bg-gray-50 rounded-full overflow-hidden"><div className="h-full rounded-full bg-indigo-500 transition-all" style={{width:`${pct}%`}}/></div></div>
                )})}</div>
              </GlassCard>
              <GlassCard className="p-5">
                <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-3">Sources</h3>
                <div className="flex gap-3">{Object.entries(modelPerf.source_breakdown||{}).map(([src,cnt]) => <div key={src} className="glass-subtle rounded-xl p-3 flex-1 text-center"><div className="text-lg font-bold text-slate-800">{cnt}</div><div className="text-[10px] text-gray-400 capitalize">{src}</div></div>)}</div>
              </GlassCard>
            </div>
          )}
          ...
          {statsSubTab === 'streams' && (
            <div className="space-y-3">
              ...
              {Object.keys(streams).length === 0 ? <GlassCard className="p-8 text-center"><p className="text-gray-400">No active streams</p><code className="text-[10px] text-gray-300 mt-1 block">POST /api/streams/add</code></GlassCard> : (
                Object.entries(streams).map(([id,s]) => (
                  <GlassCard key={id} className="p-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1"><div className="text-sm text-slate-800 font-medium">{s.name||id}</div><div className="text-[10px] text-gray-400">{s.source}</div></div>
                      <div className="text-center"><div className="text-sm font-bold text-slate-800">{s.frames||0}</div><div className="text-[9px] text-gray-400">Frames</div></div>
                      <span className={`badge ${s.status==='running'?'badge-success':'badge-danger'}`}>{s.status}</span>
                    </div>
                    {s.error && <div className="mt-2 text-xs text-red-500 bg-red-50 rounded-lg p-2">{s.error}</div>}
                  </GlassCard>
                ))
              )}
            </div>
          ...
          {statsSubTab === 'consent' && (
            <div className="space-y-4">
              {anonymization && (
                <div className="grid grid-cols-4 gap-3">
                  {[{l:'Students',v:anonymization.total_students,c:'text-slate-800'},{l:'Consented',v:anonymization.consented,c:'text-green-600'},{l:'Not Consented',v:anonymization.not_consented,c:'text-red-500'},{l:'Rate',v:`${anonymization.consent_rate}%`,c:'text-indigo-600'}].map(s=>
                    <GlassCard key={s.l} className="p-4 text-center"><div className={`text-xl font-bold ${s.c}`}>{s.v}</div><div className="text-[10px] text-gray-400 mt-0.5">{s.l}</div></GlassCard>
                  )}
                </div>
              )}
              <GlassCard className="p-5">
                <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-3">Consent Records</h3>
                {consents.length === 0 ? <p className="text-gray-400 text-center py-6">No records</p> : (
                  <div className="space-y-1.5">{consents.map(c => (
                    <div key={c.id} className="flex items-center gap-3 p-3 glass-subtle rounded-xl">
                      <div className="flex-1"><div className="text-sm text-slate-800">{c.student_name}</div><div className="text-[10px] text-gray-400">{c.student_email}</div></div>
                      ...
                      <span className="text-[10px] text-gray-300">{fmtDate(c.granted_at||c.revoked_at)}</span>
                    </div>
                  ))}</div>
                )}
              </GlassCard>
              ...
              {retention && (
                <div className="grid grid-cols-4 gap-3">
                  {[{l:'Emotion Logs',v:retention.total_emotion_logs},{l:'Sessions',v:retention.total_sessions},{l:'>30 Days',v:retention.logs_older_30_days},{l:'>90 Days',v:retention.logs_older_90_days}].map(s=>
                    <GlassCard key={s.l} className="p-4 text-center"><div className="text-xl font-bold text-slate-800">{s.v}</div><div className="text-[10px] text-gray-400 mt-0.5">{s.l}</div></GlassCard>
                  )}
                </div>
              )}
              <GlassCard className="p-5">
                <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-2">Archive Old Data</h3>
                <p className="text-gray-400 text-xs mb-3">Remove old emotion logs while keeping session summaries.</p>
                <div className="flex gap-2">{[30,60,90,180].map(d => <button key={d} onClick={() => archiveData(d)} disabled={loading} className="px-3 py-1.5 rounded-lg bg-red-50 border border-red-200 text-red-500 text-xs font-medium hover:bg-red-500/20 disabled:opacity-50 transition-all">Archive &gt; {d}d</button>)}</div>
              </GlassCard>
            </div>
          )}
          ...
          {statsSubTab === 'audit' && (
            <GlassCard className="p-5">
              ...
              {auditLogs.length === 0 ? <p className="text-gray-400 text-center py-6">No events</p> : (
                <div className="space-y-1.5 max-h-[60vh] overflow-y-auto">{auditLogs.map(l => (
                  <div key={l.id} className="flex items-center gap-3 p-3 glass-subtle rounded-xl">
                    ...
                    <div className="flex-1 min-w-0"><div className="text-xs text-slate-800"><span className="font-bold">{l.user}</span> <span className="text-indigo-600">{l.action}</span></div>{l.target && <div className="text-[10px] text-gray-400">Target: {l.target}</div>}</div>
          ...
          {statsSubTab === 'announcements' && (
            <div className="space-y-4">
              ...
              <GlassCard className="p-5">
                ...
                {announcements.length === 0 ? <p className="text-gray-400 text-center py-4">None</p> : (
                  <div className="space-y-2">{announcements.map(a => (
                    <div key={a.id} className={`p-3 rounded-xl border ${a.priority==='critical'?'bg-red-50 border-red-200':a.priority==='high'?'bg-amber-50 border-amber-200':'glass-subtle'}`}>
                      <div className="flex justify-between items-start">
                        <div><div className="text-sm text-slate-800 font-medium">{a.title}</div><div className="text-xs text-gray-500 mt-0.5">{a.content}</div><div className="text-[10px] text-gray-300 mt-1">{fmtDate(a.created_at)} · {a.priority} · {a.target_role||'All'}</div></div>
                        <button onClick={() => deleteAnnouncement(a.id)} className="p-1 text-gray-400 hover:text-red-500 transition-colors"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6m3 0V4a2 2 0 012-2h4a2 2 0 012 2v2"/></svg></button>
                      </div>
                    </div>
                  ))}</div>
                )}
