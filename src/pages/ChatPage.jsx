...
export default function ChatPage() {
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--bg-base)' }}>
      {/* Background */}
      <div className="fixed top-20 left-60 w-72 h-72 bg-indigo-600/8 rounded-full blur-[100px] pointer-events-none" />
      <div className="fixed bottom-20 right-20 w-80 h-80 bg-blue-600/6 rounded-full blur-[120px] pointer-events-none" />

      {/* Top bar */}
      <header className="glass-sidebar px-6 py-3 border-b border-white/5 flex items-center gap-4 sticky top-0 z-30">
        <Link to={backPath} className="p-2 rounded-lg text-gray-500 hover:text-white hover:bg-white/5 transition-all">
        ...
        <div className="flex items-center gap-2">
          ...
          {unreadTotal > 0 && <span className="px-1.5 py-0.5 rounded-full bg-rose-500 text-white text-[10px] font-bold">{unreadTotal}</span>}
        </div>
      </header>
          ...
          <div className="w-72 shrink-0 flex flex-col">
            <div className="glass rounded-2xl flex-1 flex flex-col overflow-hidden">
              <div className="p-3 border-b border-white/5 flex items-center justify-between">
                ...
                <button onClick={() => setShowNewChat(true)} className="w-7 h-7 rounded-lg bg-white/5 border border-white/8 text-white flex items-center justify-center hover:bg-white/10 transition-all text-sm">+</button>
              </div>
              ...
              {showNewChat && (
                <div className="p-3 border-b border-white/5 bg-white/3">
                  ...
                  <div className="mt-2 max-h-40 overflow-y-auto space-y-0.5">
                    {filteredContacts.length === 0 ? <p className="text-gray-600 text-xs text-center py-2">No contacts</p> : filteredContacts.map(c => {
                      const rc = ROLE_COLORS[c.role] || ROLE_COLORS.student
                      return (
                        <button key={c.id} onClick={() => startNewConversation(c)} className="w-full flex items-center gap-2 p-2 rounded-lg hover:bg-white/5 transition-all text-left">
                          ...
                          <div className="min-w-0 flex-1"><div className="text-white text-xs truncate">{c.name}</div><div className="text-gray-600 text-[9px] capitalize">{c.role}</div></div>
                        </button>
                      )
                    })}
                  </div>
                  <button onClick={() => setShowNewChat(false)} className="text-gray-600 text-[10px] mt-1.5 hover:text-gray-400">Cancel</button>
                </div>
              )}
              ...
              <div className="flex-1 overflow-y-auto">
                {conversations.length === 0 ? (
                  <div className="p-6 text-center">
                    ...
                    <p className="text-gray-600 text-xs mb-2">No conversations</p>
                  ...
                  return (
                    <button key={c.id} onClick={() => selectConversation(c)}
                      className={`w-full flex items-center gap-2.5 px-3 py-2.5 transition-all text-left border-l-2 ${isSelected ? 'bg-white/5 border-l-indigo-500' : 'hover:bg-white/3 border-l-transparent'}`}>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-center"><span className="text-white text-xs font-medium truncate">{c.other_user?.name}</span><span className="text-gray-700 text-[9px] flex-shrink-0">{fmtTime(c.last_at)}</span></div>
                        <div className="flex justify-between items-center mt-0.5">
                          <span className="text-gray-600 text-[10px] truncate">{c.last_message || 'No messages'}</span>
            ...
            <div className="glass rounded-2xl flex-1 flex flex-col overflow-hidden">
              {!selectedConvo ? (
                <div className="flex-1 flex flex-col items-center justify-center">
                  ...
                  <p className="text-gray-600 text-sm mb-4">Select a conversation or start a new one</p>
                ...
                <>
                  <div className="p-3 border-b border-white/5 flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-sm text-white font-semibold">{selectedConvo.other_user?.name}</h3>
                      <div className="flex items-center gap-2"><span className={`text-[9px] px-1.5 py-0.5 rounded ${ROLE_COLORS[selectedConvo.other_user?.role]?.badge || 'badge-info'} capitalize`}>{selectedConvo.other_user?.role}</span><span className="text-gray-700 text-[10px]">{selectedConvo.other_user?.email}</span></div>
                    </div>
                  </div>

                  <div className="flex-1 overflow-y-auto p-4 space-y-2">
                    {messages.length === 0 ? <div className="text-center py-12 text-gray-600 text-xs">Start the conversation!</div> : messages.map(m => (
                      <div key={m.id} className={`flex ${m.is_mine ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-[70%] px-3.5 py-2.5 rounded-2xl text-sm ${m.is_mine ? 'bg-gradient-to-r from-indigo-500 to-blue-500 text-white rounded-br-md' : 'bg-white/8 text-gray-200 rounded-bl-md'}`}>
                          <div className="whitespace-pre-wrap break-words">{m.content}</div>
                          <div className={`text-[9px] mt-1 flex items-center gap-1 ${m.is_mine ? 'text-indigo-200 justify-end' : 'text-gray-600'}`}>
                  ...
                  <div className="p-3 border-t border-white/5">
                    ...
                    <div className="text-[9px] text-gray-700 mt-1 text-right">Enter to send · Shift+Enter for new line</div>
                  </div>
                </>
              )}
            </div>
