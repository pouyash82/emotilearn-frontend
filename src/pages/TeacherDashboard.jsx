...
export default function TeacherDashboard() {
  ...
  return (
    <DashboardLayout activeTab={tab} onTabChange={setTab} title={`Dashboard / ${user?.name || 'Teacher'}`}
        ...
        <div className="space-y-6 animate-fade-in">
          ...
          <div className="grid grid-cols-3 gap-4">
            {[
              ...
              <GlassCard key={s.label} variant="stat" accent={s.accent} className="p-5">
                ...
                <div className="text-3xl font-bold text-slate-800 mt-2">{s.value}</div>
              </GlassCard>
            ))}
          </div>

          <GlassCard className="p-6">
            <h3 className="text-base font-semibold text-slate-800 mb-5">Class Engagement Overview</h3>
            {students.length === 0 ? (
              ...
              <div className="grid grid-cols-3 gap-5">
                ...
                <div className="glass-subtle rounded-xl p-4">
                  ...
                  <div className="mt-3 space-y-2">
                    {topEmotions.length === 0 ? <p className="text-gray-400 text-sm">No data</p> : topEmotions.map(([emo, cnt]) => (
                  ...
                  <div className="mt-3 space-y-2">
                    {students.filter(s => s.total_sessions > 0).sort((a,b) => new Date(b.last_active||0) - new Date(a.last_active||0)).slice(0, 4).map(s => (
                      <div key={s.id} className="flex items-center gap-2">
                        <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-indigo-100 to-blue-100 flex items-center justify-center text-slate-800 text-[10px] font-bold">{s.name?.charAt(0)||'?'}</div>
                        <div className="min-w-0 flex-1"><div className="text-sm text-slate-800 truncate">{s.name}</div><div className="text-[10px] text-gray-400">{fmtDate(s.last_active)}</div></div>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          <div className="grid grid-cols-4 gap-3">
            {[
              ...
              <button key={a.label} onClick={a.action} className="glass-subtle hover:bg-gray-50 rounded-xl p-5 text-center transition-all group">
                ...
                <div className="text-sm text-gray-400 font-medium group-hover:text-slate-800 transition-colors">{a.label}</div>
              </button>
            ))}
          </div>
        </div>
      ...
      {tab === 'students' && (
        <GlassCard className="p-6 animate-fade-in">
          <div className="flex items-center justify-between mb-5">
            <h3 className="text-base font-semibold text-slate-800">Student Progress</h3>
            <span className="text-xs text-gray-400">Click a student to view details</span>
          </div>
          {students.length === 0 ? (
              ...
              {students.map(s => {
                ...
                return (
                  <button key={s.id} onClick={() => setSelectedStudent(s)}
                    className="w-full text-left flex items-center gap-4 p-4 rounded-xl glass-subtle hover:bg-gray-50 hover:border-indigo-200 transition-all">
                    <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-100 to-blue-100 border border-indigo-200 flex items-center justify-center text-slate-800 text-sm font-bold flex-shrink-0">{s.name?.charAt(0)||'?'}</div>
                    <div className="flex-1 min-w-0">
                      <div className="text-sm text-slate-800 font-medium flex items-center gap-2">{s.name}
                        {(s.total_sessions || 0) > 0 && <span className="badge badge-success text-[9px]">Active</span>}
                      </div>
                      <div className="text-xs text-gray-400">{s.email}{s.last_active && ` · Last: ${fmtDate(s.last_active)}`}</div>
                    </div>
                    <div className="flex gap-6 flex-shrink-0">
                      <div className="text-center"><div className="text-sm font-bold text-slate-800">{s.total_sessions||0}</div><div className="text-[10px] text-gray-400">Sessions</div></div>
                      <div className="text-center"><div className="text-sm font-bold" style={{color:c}}>{s.avg_engagement||0}%</div><div className="text-[10px] text-gray-400">Engage</div></div>
                      <div className="text-center"><div className="text-sm font-bold text-blue-600 capitalize">{s.dominant_emotion||'—'}</div><div className="text-[10px] text-gray-400">Dominant</div></div>
                    </div>
                    <div className="w-24 flex-shrink-0"><div className="h-1.5 bg-gray-50 rounded-full overflow-hidden"><div className="h-full rounded-full transition-all" style={{width:`${s.avg_engagement||0}%`, background:c}}/></div></div>
                  </button>
                )
              })}
            </div>
      ...
      {(tab === 'courses' || tab === 'exams') && (
        <div className="space-y-5 animate-fade-in">
          ...
          <GlassCard className="p-5">
            ...
            {courses.length === 0 ? <p className="text-gray-400 text-center py-8">No courses yet</p> : (
              <div className="space-y-3">
                {courses.map(c => (
                  <div key={c.id} className="glass-subtle rounded-xl overflow-hidden">
                    <div className="p-4 flex items-center justify-between cursor-pointer hover:bg-gray-50 transition-all" onClick={() => toggleCourse(c.id)}>
                      <div>
                        <h4 className="text-slate-800 font-semibold">{c.name}</h4>
                        <p className="text-gray-400 text-xs">{c.description || 'No description'} · {c.students?.length || 0} students</p>
                      </div>
                      <div className="flex items-center gap-2">
                        <button onClick={e => { e.stopPropagation(); deleteCourse(c.id) }} className="p-1.5 rounded-lg text-gray-400 hover:text-red-500 hover:bg-red-50 transition-all">
                    ...
                    {expandedCourse === c.id && (
                      <div className="border-t border-gray-200 p-4 space-y-3">
                        {!showExamForm ? (
                          <button onClick={() => setShowExamForm(true)} className="w-full py-2.5 rounded-xl bg-indigo-50 border border-indigo-200 text-indigo-600 text-sm font-medium hover:bg-indigo-100 transition-all">+ Create Exam</button>
                        ) : (
                          <div className="glass-subtle rounded-xl p-4 space-y-3">
                            <h4 className="text-slate-800 font-semibold text-sm">New Exam</h4>
                            ...
                            <div className="flex gap-3">
                              ...
                              <div className="flex items-center gap-2"><span className="text-gray-400 text-xs">Time:</span>
                                <input type="number" value={examForm.time_limit} onChange={e => setExamForm(f => ({...f, time_limit: parseInt(e.target.value)||10}))} className="w-14 input-glass text-sm text-center" /><span className="text-gray-400 text-xs">min</span></div>
                            </div>
                            <div className="space-y-2 max-h-56 overflow-y-auto">
                              {examForm.questions.map((q, qi) => (
                                <div key={qi} className="bg-gray-50 rounded-lg p-3 border border-gray-200">
                                  <div className="flex justify-between items-center mb-1.5"><span className="text-gray-500 text-[10px] font-bold">Q{qi+1}</span>{examForm.questions.length > 1 && <button onClick={() => removeQuestion(qi)} className="text-red-500 text-[10px]">Remove</button>}</div>
                                  <input type="text" placeholder="Question" value={q.q} onChange={e => updateQuestion(qi,'q',e.target.value)} className="w-full px-3 py-1.5 rounded-lg bg-gray-50 border border-gray-200 text-slate-800 text-xs mb-1.5 focus:outline-none" />
                                  <div className="grid grid-cols-2 gap-1.5">
                                    {q.o.map((opt, oi) => (
                                      <div key={oi} className="flex items-center gap-1">
                                        <input type="radio" name={`ans-${qi}`} checked={q.a===oi} onChange={() => updateQuestion(qi,'a',oi)} className="accent-indigo-500" />
                                        <input type="text" placeholder={`Option ${String.fromCharCode(65+oi)}`} value={opt} onChange={e => updateQuestion(qi,String(oi),e.target.value)} className="flex-1 px-2 py-1 rounded bg-gray-50 border border-gray-200 text-slate-800 text-[11px] focus:outline-none" />
                                      </div>
                                    ))}
                                  </div>
                                </div>
                            ...
                            <button onClick={addQuestion} className="text-xs text-indigo-600 hover:text-indigo-500">+ Add Question</button>
                        ...
                        {courseExams.length === 0 ? <p className="text-gray-300 text-xs text-center py-2">No exams yet</p> : (
                          <div className="space-y-1.5">
                            {courseExams.map(ex => (
                              <div key={ex.id} className="flex items-center justify-between glass-subtle rounded-lg p-3">
                                <div><div className="text-sm text-slate-800 font-medium">{ex.is_proctored && <span className="text-red-500 text-[10px] mr-1">Proctored</span>}{ex.title}</div><div className="text-gray-400 text-xs">{ex.question_count}q · {Math.floor(ex.time_limit/60)}min</div></div>
                                <button onClick={() => loadSubmissions(ex.id, ex.title)} className="px-3 py-1 rounded-lg bg-indigo-50 text-indigo-600 text-xs font-medium hover:bg-indigo-100 transition-all">Submissions</button>
                              </div>
                            ))}
                          </div>
                        )}
          ...
          <div className="flex gap-2">
            {analyticsSubTabs.map(t => (
              <button key={t.id} onClick={() => setAnalyticsSubTab(t.id)}
                className={`px-4 py-2 rounded-xl text-xs font-medium transition-all ${analyticsSubTab === t.id ? 'bg-indigo-50 text-indigo-600 border border-indigo-200' : 'bg-gray-50 text-gray-500 border border-gray-200 hover:bg-gray-50'}`}>
                {t.label}
              </button>
            ))}
          </div>
      ...
      {viewingSubs && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => { setViewingSubs(null); setViewingDetail(null) }}>
            ...
            {!viewingDetail ? (
              <>
                <div className="flex justify-between items-center mb-5">
                  <div><h3 className="text-slate-800 font-bold text-lg">{viewingSubs.title}</h3><p className="text-gray-500 text-xs">{viewingSubs.total} submissions · Avg: {viewingSubs.avg_score}% · Focus: {viewingSubs.avg_focus}%</p></div>
                  <button onClick={() => { setViewingSubs(null); setViewingDetail(null) }} className="p-1.5 rounded-lg text-gray-500 hover:text-slate-800 hover:bg-gray-100 transition-all">
                ...
                {submissions.length === 0 ? <p className="text-gray-400 text-center py-8">No submissions yet</p> : (
                  <div className="space-y-2">
                    {submissions.map(s => (
                      <div key={s.id} onClick={() => loadSubmissionDetail(s.id)} className="flex items-center justify-between glass-subtle rounded-xl p-4 cursor-pointer hover:bg-gray-50 transition-all">
                        <div><div className="text-slate-800 font-medium text-sm">{s.student_name}</div><div className="text-gray-400 text-xs">{s.student_email} · {s.submitted_at ? new Date(s.submitted_at).toLocaleString() : ''}</div></div>
                        <div className="flex gap-4 text-sm">
                          <div className="text-center"><div className="font-bold" style={{color: s.score>=70?'#22c55e':s.score>=50?'#eab308':'#ef4444'}}>{s.score}%</div><div className="text-[10px] text-gray-400">Score</div></div>
                          <div className="text-center"><div className="font-bold" style={{color: s.focus_score>=85?'#22c55e':s.focus_score>=50?'#eab308':'#ef4444'}}>{s.focus_score}%</div><div className="text-[10px] text-gray-400">Focus</div></div>
                          <div className="text-center"><div className="font-bold text-amber-600">{s.alerts_count}</div><div className="text-[10px] text-gray-400">Alerts</div></div>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <>
                <div className="flex justify-between items-center mb-4">
                  <div><h3 className="text-slate-800 font-bold">{viewingDetail.student?.name}'s Submission</h3><p className="text-gray-400 text-xs">{viewingDetail.exam?.title}</p></div>
                  <button onClick={() => setViewingDetail(null)} className="text-indigo-600 text-xs hover:text-indigo-500">&larr; Back</button>
                </div>
                <div className="grid grid-cols-3 gap-3 mb-4">
                  <div className="glass-subtle rounded-xl p-3 text-center"><div className="text-xl font-bold" style={{color:viewingDetail.submission?.score>=70?'#22c55e':'#ef4444'}}>{viewingDetail.submission?.score}%</div><div className="text-[10px] text-gray-400">Score</div></div>
                  <div className="glass-subtle rounded-xl p-3 text-center"><div className="text-xl font-bold" style={{color:viewingDetail.submission?.focus_score>=85?'#22c55e':'#eab308'}}>{viewingDetail.submission?.focus_score}%</div><div className="text-[10px] text-gray-400">Focus</div></div>
                  <div className="glass-subtle rounded-xl p-3 text-center"><div className="text-xl font-bold text-slate-800">{Math.floor((viewingDetail.submission?.duration_sec||0)/60)}:{((viewingDetail.submission?.duration_sec||0)%60).toString().padStart(2,'0')}</div><div className="text-[10px] text-gray-400">Duration</div></div>
                </div>
                <a href={`${BACKEND}/api/exam/video/${viewingDetail.submission?.id||0}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-indigo-50 border border-indigo-200 text-indigo-600 text-sm hover:bg-indigo-100 transition-all mb-4">Download Recording</a>
...
function StudentAnalyticsModal({ student, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/30 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in" onClick={onClose}>
      <div className="glass-heavy rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="p-5 border-b border-gray-200 flex items-center justify-between sticky top-0 z-10" style={{background:'rgba(17,24,39,0.95)', backdropFilter:'blur(20px)'}}>
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-indigo-100 to-blue-100 border border-indigo-200 flex items-center justify-center text-slate-800 text-lg font-bold">{student.name?.charAt(0)||'?'}</div>
            <div><h2 className="text-lg font-bold text-slate-800">{student.name}</h2><p className="text-gray-400 text-xs">{student.email}</p></div>
          </div>
        ...
        <div className="p-5 space-y-5">
          <div className="grid grid-cols-3 gap-3">
            <div className="glass-subtle rounded-xl p-4 text-center"><div className="text-2xl font-bold text-slate-800">{student.total_sessions||0}</div><div className="text-xs text-gray-500 mt-0.5">Sessions</div></div>
            ...
            <div className="glass-subtle rounded-xl p-4 text-center"><div className="text-2xl font-bold text-blue-600 capitalize">{student.dominant_emotion||'N/A'}</div><div className="text-xs text-gray-500 mt-0.5">Dominant</div></div>
          </div>

          {loading ? <p className="text-gray-400 text-sm">Loading...</p> : Object.keys(dist).length > 0 && (
            <div>
              ...
              <div className="mt-2 space-y-1.5">
                {Object.entries(dist).sort((a,b) => b[1]-a[1]).map(([emo, pct]) => (
                  <div key={emo}><div className="flex justify-between text-xs mb-0.5"><span className="text-gray-400 capitalize">{emo}</span><span className="text-slate-800 font-bold">{pct}%</span></div>
                    <div className="h-1.5 bg-gray-50 rounded-full overflow-hidden"><div className="h-full rounded-full transition-all duration-700" style={{width:`${pct}%`, background:EMOTION_COLORS[emo]||'#7c3aed'}}/></div></div>
                ))}
              </div>
            </div>
          )}
          ...
          <div>
            ...
            {loading ? <p className="text-gray-400 text-sm mt-2">Loading...</p> : sessions.length === 0 ? <p className="text-gray-400 text-sm mt-2">No sessions recorded.</p> : (
              <div className="mt-2 space-y-1.5">
                {sessions.map(s => {
                  const sc = engColor(s.avg_engagement||0)
                  return (
                    <div key={s.id} className="flex items-center gap-3 p-3 glass-subtle rounded-xl">
                      <div className="w-8 h-8 rounded-lg bg-indigo-50 flex items-center justify-center text-slate-800 text-[10px] font-bold flex-shrink-0">#{s.id}</div>
                      <div className="flex-1 min-w-0"><div className="text-xs text-slate-800">{fmtDate(s.started_at)}</div><div className="text-[10px] text-gray-400">{s.total_detections||0} detections</div></div>
                      ...
                      <div className="flex gap-1.5 flex-shrink-0">
                        <button onClick={() => openReport(s.id)} disabled={downloading===`r-${s.id}`} className="px-2 py-1 rounded-lg bg-indigo-50 text-indigo-600 text-[10px] font-medium hover:bg-indigo-100 disabled:opacity-50 transition-all">Report</button>
                        <button onClick={() => downloadCSV(s.id)} disabled={downloading===`c-${s.id}`} className="px-2 py-1 rounded-lg bg-gray-50 text-gray-400 text-[10px] font-medium hover:bg-gray-100 disabled:opacity-50 transition-all">CSV</button>
                      </div>
                    </div>
                  )
                })}
