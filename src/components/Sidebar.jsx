...
const icons = {
  ...
  live: (<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="3"/><path d="M16.95 7.05a7 7 0 0 1 0 9.9"/><path d="M7.05 16.95a7 7 0 0 1 0-9.9"/><path d="M19.78 4.22a11 11 0 0 1 0 15.56"/><path d="M4.22 19.78a11 11 0 0 1 0-15.56"/></svg>),
  ...
  compare: (<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/></svg>),
  ...
  analytics: (<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 12c0 4.97-4.03 9-9 9s-9-4.03-9-9 4.03-9 9-9"/><path d="M21 3l-9 9"/><path d="M21 3h-6"/><path d="M21 3v6"/></svg>),
  ...
  exam: (<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg>),
}

const NAV_CONFIGS = {
  student: { label: 'STUDENT', items: [
...
export default function Sidebar({ activeTab, onTabChange, unreadCount = 0 }) {
  ...
  return (
    <aside className="fixed left-0 top-0 bottom-0 glass-sidebar z-40 flex flex-col" style={{ width:'var(--sidebar-width)' }}>
      <div className="px-5 py-5 border-b border-gray-200/60">
        <Link to="/" className="flex items-center gap-3 group">
          <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-indigo-500 to-blue-500 flex items-center justify-center shadow-md shadow-indigo-200 group-hover:shadow-indigo-300 group-hover:scale-105 transition-all duration-300">
      ...
      <div className="px-5 pt-5 pb-2">
        <span className="text-[10px] font-semibold tracking-[0.15em] text-gray-400 uppercase">{config.label}</span>
      </div>

      <nav className="flex-1 px-3 space-y-0.5 overflow-y-auto">
        {config.items.map((item) => (
          <button key={item.id} onClick={() => onTabChange(item.id)} className={`sidebar-nav-item w-full ${activeTab === item.id ? 'active' : ''}`}>
            <span className="flex-shrink-0 w-5 h-5 flex items-center justify-center">{icons[item.icon]}</span>
            <span className="flex-1 text-left">{item.label}</span>
            {item.id === 'notifications' && unreadCount > 0 && (
              <span className="min-w-[20px] h-5 flex items-center justify-center rounded-full bg-rose-500 text-white text-[10px] font-bold px-1.5">{unreadCount > 99 ? '99+' : unreadCount}</span>
            )}
          </button>
        ))}
      </nav>
      ...
      <div className="border-t border-gray-200/60 p-3 space-y-1">
        <Link to="/chat" className="sidebar-nav-item w-full">
          <span className="flex-shrink-0 w-5 h-5 flex items-center justify-center">{icons.chat}</span>
        ...
        <div className="flex items-center gap-3 px-4 py-3">
          ...
          <button onClick={handleLogout} className="p-1.5 rounded-lg text-gray-400 hover:text-red-500 hover:bg-red-50 transition-all" title="Logout">{icons.logout}</button>
        </div>
      </div>
    </aside>
  )
