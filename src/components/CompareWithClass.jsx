    ...
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="grid grid-cols-3 gap-4">
        <GlassCard variant="stat" accent="indigo" className="p-5 text-center">
          ...
          <div className={`text-xs font-semibold mt-2 px-2.5 py-1 rounded-lg inline-block ${engDiff >= 0 ? 'bg-green-50 text-green-600 border border-green-500/15' : 'bg-red-50 text-red-500 border border-red-200'}`}>
        ...
        <GlassCard variant="stat" accent="teal" className="p-5 text-center">
          ...
          <div className="text-xs text-gray-400 mt-2">{class_average.total_students} students</div>
        </GlassCard>
        ...
        <GlassCard variant="stat" accent="amber" className="p-5 text-center">
          ...
          <div className="text-xs text-gray-400 mt-2">of {class_average.total_students} students</div>
        </GlassCard>
      </div>
      ...
      <GlassCard className="p-5">
        ...
        <div className="space-y-4">
          <div>
            <div className="flex justify-between text-xs mb-1"><span className="text-indigo-600 font-medium">You</span><span className="text-indigo-600 font-bold">{student.avg_engagement}%</span></div>
            <div className="h-3 bg-gray-50 rounded-full overflow-hidden"><div className="h-full rounded-full bg-gradient-to-r from-indigo-500 to-blue-500 transition-all duration-1000" style={{ width: `${Math.min(100, student.avg_engagement)}%` }} /></div>
          </div>
          <div>
            ...
            <div className="h-3 bg-gray-50 rounded-full overflow-hidden"><div className="h-full rounded-full bg-gradient-to-r from-gray-600 to-gray-500 transition-all duration-1000" style={{ width: `${Math.min(100, class_average.avg_engagement)}%` }} /></div>
          </div>
        </div>
      </GlassCard>
            ...
            return (
              <div key={emo}>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-xs text-slate-800 font-medium capitalize w-20">{emo}</span>
                  <span className="text-indigo-600 text-xs font-bold ml-auto">{myPct}%</span>
                  <span className="text-gray-400 text-xs">vs</span>
                  <span className="text-gray-500 text-xs font-bold">{classPct}%</span>
                </div>
                <div className="flex gap-1">
                  <div className="flex-1 h-2 bg-gray-50 rounded-full overflow-hidden"><div className="h-full rounded-full transition-all duration-700" style={{ width: `${(myPct / maxPct) * 100}%`, background: EMOTION_COLORS[emo] }} /></div>
                  <div className="flex-1 h-2 bg-gray-50 rounded-full overflow-hidden"><div className="h-full rounded-full transition-all duration-700" style={{ width: `${(classPct / maxPct) * 100}%`, background: EMOTION_COLORS[emo], opacity: 0.3 }} /></div>
                </div>
                <div className="flex text-[9px] text-gray-300 mt-0.5"><span className="flex-1">You</span><span className="flex-1">Class</span></div>
              </div>
            )
          })}
        </div>
      ...
      <GlassCard className="p-5">
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            { label: 'Your Sessions', value: student.total_sessions, color: 'text-slate-800' },
            { label: 'Class Total', value: class_average.total_sessions, color: 'text-gray-400' },
            { label: 'Your Detections', value: student.total_detections, color: 'text-slate-800' },
          ...
          <div className="flex items-end gap-1.5 h-28">
            {student.trend.map((p, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-0.5">
                <span className="text-[9px] text-gray-400">{p.engagement}%</span>
                ...
                <span className="text-[8px] text-gray-300">{p.date ? new Date(p.date).toLocaleDateString('en', { month:'short', day:'numeric' }) : ''}</span>
              </div>
            ))}
          </div>
